...
export default class AddCourse extends React.Component {
    constructor(props) {
        ...
        this.data=fromJS({
            ...
            arrangements:[
                {id:300,key:1,termWeek:"第1周",week:"周1",lesson:"1-2节",arrangementName:"TCP/IP原理"}
            ]
        })//immutable
        /* fake date use in test end*/
        let addOrEditArrangementData=fromJS({id:-1,key:-1,termWeek:"",week:"",lesson:"",arrangementName:""});
        this.state={currentStep:1,courseData:this.data,showAddArrangmentModal:false,
            showEditArrangmentModal:false,
    ...
    handleShowAddArrangmentModal(){
        let addOrEditArrangementData=fromJS({id:-1,key:-1,termWeek:"未选择",week:"未选择",lesson:"未选择",arrangementName:""});
        this.setState({addOrEditArrangementData:addOrEditArrangementData})
        this.setState({showAddArrangmentModal:true});
    }
    handleHideAddArrangmentModal(){
        this.setState({showAddArrangmentModal:false});
    }
    ...
    handleAddArrangmentConfirm(arrangement){ //arg is immutable data
        this.handleHideAddArrangmentModal();
        ...
        newArrangement=newArrangement.map((item,index)=>item.set("key",index));
        courseData=courseData.set("arrangements",newArrangement);
        this.setState({courseData:courseData});
    }
    handleAddArrangmentCancel(){
        this.handleHideAddArrangmentModal();
    }
    ...
    handleEditArrangmentConfirm(arrangement){
        ...
        originArrangement=originArrangement.map((item)=>{
            if(item.get("key")==arrangement.get("key")){
                //item=item.set("id",arrangement.get("id"));
                //item=item.set("termWeek",arrangement.get("termWeek"));
                //item=item.set("week",arrangement.get("week"));
                //item=item.set("lesson",arrangement.get("lesson"));
                //item=item.set("arrangementName",arrangement.get("arrangementName"));
                //使用如下方法创造一个新的arrangment对象，这个位置可能是一个出现bug的地方，在未来测试时注意
                ...
                render: (text,record) => (
                    <div>
                        <a type="primary" size="small"
                           onClick={(event)=>{this.handleEditArrangment(record.key);}}>编辑 </a >
        ...
        return(
            <div>
                <Row type="flex" justify="end" align="middle" style={{marginTop:"20px"}}>
                    <Button type="primary" onClick={()=>{this.handleShowAddArrangmentModal()}}>添加</Button>
                </Row>
                ...
                <AddOrEditArrangmentModal title="添加课程"
                    visible={this.state.showAddArrangmentModal} dataSource={this.state.addOrEditArrangementData} //pass immutable datasource
                    course={this.state.courseData} //传入课程信息数据结构，这样可以判断是实验还是非实验课
                    confirm={(v)=>{this.handleAddArrangmentConfirm(v)}} cancel={()=>{this.handleAddArrangmentCancel()}}></AddOrEditArrangmentModal>
